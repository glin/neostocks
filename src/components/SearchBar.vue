<template>
  <b-nav-form class="search-form" @submit="handleSubmit" @keydown.esc="showResults(false)">
    ...
    <b-list-group v-if="resultsVisible" ref="searchResults" class="search-results" @mouseout="handleResultsHover(-1)">
      <b-list-group-item v-for="(company, index) in results" ref="searchResultItems" :key="company.ticker" :to="company.href" :class="{ hover: isSelected(index) }" class="search-result-item" active-class="test" tabindex="-1" @mouseover.native="handleResultsHover(index)" @click.native="handleSubmit" @mousedown.prevent.native>
